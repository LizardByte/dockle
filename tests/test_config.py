...
class ConfigTests(unittest.TestCase):
    def test_loads_defaults_and_resolves_paths(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(config.targets[0].output, root.resolve() / "_site" / "manual")
    ...
    def test_rejects_unknown_keys(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            contents = MINIMAL_CONFIG.replace('name = "Example"', 'name = "Example"\ncolour = "blue"')
    ...
    def test_rejects_theme_values_that_could_escape_css(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "dockle.toml"
            path.write_text(
                MINIMAL_CONFIG.replace("[[targets]]", '[theme]\nprimary = "red; display: none"\n\n[[targets]]'),
                encoding="utf-8",
            )
...
""",
    ...
    def test_rejects_overlapping_target_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "dockle.toml"
            path.write_text(
                MINIMAL_CONFIG.replace('name = "manual"', 'name = "manual"\noutput = "reference"')
                + """
[[targets]]
name = "api"
framework = "doxygen"
...
""",
    ...
    def test_rejects_entry_outside_source(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "dockle.toml"
            path.write_text(
                MINIMAL_CONFIG.replace('source = "docs"', 'source = "docs"\nentry = "../README"'),
                encoding="utf-8",
            )

            with self.assertRaisesRegex(ConfigError, "entry must be a relative path"):
                load_config(path)
...
""",
            ...
            self.assertEqual([target.name for target in selected], ["manual", "api"])

    def test_allows_project_root_as_source(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "dockle.toml"
            path.write_text(
                MINIMAL_CONFIG.replace('framework = "sphinx"', 'framework = "rustdoc"').replace(
                    'source = "docs"', 'source = "."'
                ),
                encoding="utf-8",
            )
