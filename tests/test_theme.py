...
class ThemeTests(unittest.TestCase):
    def test_render_theme_includes_configured_tokens(self) -> None:
        stylesheet = render_theme(ThemeConfig(primary="#abcdef", dark_background="#010203"))
    ...
    def test_apply_theme_uses_relative_asset_paths_for_nested_pages(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            count = apply_theme(output, "rustdoc", "body { color: red; }", portal=output.parent)
    ...
    def test_apply_theme_rejects_malformed_html(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            output = Path(directory)
            (output / "index.html").write_text("<html><body></body></html>", encoding="utf-8")

            with self.assertRaisesRegex(ThemeError, "closing head"):
                apply_theme(output, "jsdoc", "body {}")

    def test_native_theme_gets_framework_marker_without_duplicate_stylesheet(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            output = Path(directory)
            html = output / "index.html"
            html.write_text(
